...
import {
  Banknote, Upload, CheckCircle, Clock, Shield, Phone,
  Camera, X, Loader2, AlertCircle, ArrowRight, DollarSign, Lock
} from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight mb-4 tracking-tight">
                Need Cash?
                <span className="text-amber-400"> Get a Pawn Loan</span>
              </h1>
              <p className="text-gray-400 text-lg mb-8">Bring in your valuables and get cash on the spot. Pay back the loan to get your items back.</p>

              <div className="space-y-5 mb-10">
                {[
                  { icon: <DollarSign className="w-5 h-5" />, title: 'Fast cash — same day approval', desc: 'Walk in with your item, walk out with cash' },
                  { icon: <Lock className="w-5 h-5" />, title: 'Your items stay safe', desc: 'Stored securely until you\'re ready to pick up' },
                  { icon: <Clock className="w-5 h-5" />, title: 'Fair rates & flexible terms', desc: 'Competitive loan amounts with reasonable terms' },
                  { icon: <Shield className="w-5 h-5" />, title: 'No credit check required', desc: 'Your valuables are your collateral' },
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className="text-amber-400 mt-0.5">{item.icon}</div>
                    <div>
              ...
              <div className="bg-[#060606] border border-white/[0.06] rounded-xl p-5">
                <p className="text-white font-semibold text-sm mb-2">What can I pawn?</p>
                <p className="text-gray-500 text-sm">Firearms, jewelry, watches, electronics, tools, musical instruments, and more. If it has value, we\'ll make you an offer.</p>
                <a href="tel:3862264653" className="text-red-400 text-sm mt-3 inline-flex items-center gap-1.5 hover:text-red-300 transition-colors">
                  <Phone className="w-3.5 h-3.5" /> Questions? Call (386) 226-4653
                </a>
              </div>
            </motion.div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
            <form onSubmit={handleSubmit} className="bg-[#060606] border border-white/[0.06] rounded-2xl p-6 sm:p-8 space-y-5">
              <h2 className="text-white font-bold text-lg">Request a Pawn Loan</h2>
              ...
              <div>
                ...
                <textarea name="description" value={form.description} onChange={handleChange} rows={3} className="w-full bg-black border border-white/[0.08] rounded-lg px-4 py-3 text-sm text-white placeholder:text-gray-700 resize-none" placeholder="Describe your item: brand, model, condition, any relevant details..." />
              </div>
              ...
              >
                {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting...</> : <><ArrowRight className="w-4 h-4" /> Submit Pawn Request</>}
              </button>

              <p className="text-gray-700 text-[10px] text-center">No credit check. No obligation. We respond within 24 hours.</p>
            </form>
          </motion.div>
        </div>
      </div>
