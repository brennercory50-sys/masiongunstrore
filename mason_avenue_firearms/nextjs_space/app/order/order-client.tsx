...
import {
  ...
  ArrowRight, Truck, Clock, Shield, Package, Store,
  Globe, CreditCard, MapPin, FileCheck, ChevronRight
} from 'lucide-react';
...
const popularBrands = [
  'Glock', 'SIG Sauer', 'Smith & Wesson', 'Colt', 'Springfield', 'Beretta', 'Ruger', 'Henry', 'Kimber', 'FN', 'CZ', 'Walther'
];
      ...
      <div className="min-h-screen bg-black">
        ...
        <div className="pt-32 pb-16 max-w-lg mx-auto px-4 text-center">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5 }}>
            ...
            <p className="text-gray-600 text-xs mb-8">We respond to every request — check your phone and email.</p>
            <a href="tel:3862264653" className="inline-flex items-center gap-2 text-red-400 text-sm hover:text-red-300 transition-colors">
              <Phone className="w-4 h-4" /> Need faster help? Call (386) 226-4653
            </a>
          </motion.div>
        </div>
        <Footer />
        ...
        <div className="text-center mb-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight mb-4 tracking-tight">
              Can&apos;t Find It?
              <span className="text-red-500"> We&apos;ll Source It</span>
            </h1>
            <p className="text-gray-400 text-lg max-w-2xl mx-auto mb-8">
              We have access to distributor networks most shooters can&apos;t touch. Tell us what you want — we&apos;ll find it and get you a fair price.
            </p>

            {/* Trust badges */}
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
              {[
                { icon: <Globe className="w-4 h-4" />, label: 'Dealer network access' },
                { icon: <MapPin className="w-4 h-4" />, label: 'Local pickup' },
                { icon: <Shield className="w-4 h-4" />, label: 'FFL compliant' },
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {processSteps.map((step, i) => (
              <div key={i} className="bg-[#060606] border border-white/[0.06] rounded-xl p-6 text-center">
                <div className="w-10 h-10 bg-red-500/10 border border-red-500/20 rounded-full flex items-center justify-center text-red-400 font-bold text-sm mx-auto mb-4">
                  {step.num}
                </div>
          ...
          <div className="bg-[#060606] border border-white/[0.06] rounded-xl p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {popularBrands.map((brand, i) => (
                <span key={i} className="text-gray-400 text-xs bg-black border border-white/[0.06] px-3 py-1.5 rounded-lg">
                  {brand}
                </span>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
            <form onSubmit={handleSubmit} className="bg-[#060606] border border-white/[0.06] rounded-2xl p-6 sm:p-8 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[10px] text-gray-600 uppercase tracking-[0.15em] mb-1.5 block font-medium">Budget Range</label>
                  <input name="budget" value={form.budget} onChange={handleChange} className="w-full bg-black border border-white/[0.08] rounded-lg px-4 py-3 text-sm text-white placeholder:text-gray-700" placeholder="$500 - $1,500" />
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-gray-600 uppercase tracking-[0.15em] mb-1.5 block font-medium">Brand (if known)</label>
                ...
                <div>
                  <label className="text-[10px] text-gray-600 uppercase tracking-[0.15em] mb-1.5 block font-medium">Model (if known)</label>
              ...
              <div>
                ...
                <textarea name="description" value={form.description} onChange={handleChange} rows={3} className="w-full bg-black border border-white/[0.08] rounded-lg px-4 py-3 text-sm text-white placeholder:text-gray-700 resize-none" placeholder="Brand, model, caliber, finish, features, accessories... any details help us find it faster." />
              </div>
              ...
              <div className="flex items-center justify-center gap-2 text-gray-600 text-xs">
                ...
                We respond to every request. Local pickup only — no shipping.
              </div>
            </form>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} className="lg:sticky lg:top-28 lg:self-start space-y-6">
            ...
            <div className="bg-[#060606] border border-white/[0.06] rounded-xl p-5">
              ...
              <p className="text-gray-500 text-sm mb-4">We&apos;re experts at tracking down hard-to-find firearms. Call us with your requirements.</p>
              <a href="tel:3862264653" className="inline-flex items-center gap-2 text-red-500 hover:text-red-400 transition-colors text-sm font-medium">
            ...
            <div className="bg-[#0a0a0a] border border-white/[0.06] rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-1">All firearms transferred through our licensed FFL. Valid ID required. Transfer fees apply.</p>
                </div>
              </div>
            </div>
          </motion.div>
